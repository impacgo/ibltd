...
const StripeSetupForm = ({
  ...
  setupProcessing,
  userToken,
  isSetup = true, // true = SetupIntent (saving card), false = PaymentIntent (one‑time)
}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let result;
      if (isSetup) {
        // For SetupIntent (saving card)
        result = await stripe.confirmSetup({
          elements,
          redirect: "if_required",
        });
      } else {
        // For PaymentIntent (authorising now, not saving)
      ...
      // Pass the intent and the consent flag (for SetupIntent only)
